...
eff_px_size_nm = (L/np.sqrt(2))/(np.sqrt(K)-1) # if each acquisition corresponded to a px (for example in a raster scan image render)
total_size = np.sqrt(K) * eff_px_size_nm # total size in effective pixels
...
λplot = ax.imshow(λ.reshape(8, 8), interpolation='None', cmap='gray',
...
ax.plot(x, n_array.reshape(8, 8)[5, :], '-s', label='Data')
ax.plot(x, λ.reshape(8, 8)[5, :], label='λ (ground truth)')
ax.plot(x, λmle.reshape(8, 8)[5, :], label='$λ_{MLE}$')
